...
void read_sei_end_bits(h264_stream_t* h, bs_t* b )
{
    ...
    if ( !h264_bs_byte_aligned( b ) )
    {
        if ( !h264_bs_read_u1( b ) ) fprintf(stderr, "WARNING: bit_equal_to_one is 0!!!!\n");
        while ( ! h264_bs_byte_aligned( b ) )
        {
            if ( h264_bs_read_u1( b ) ) fprintf(stderr, "WARNING: bit_equal_to_zero is 1!!!!\n");
        }
    }
...
void h264_read_sei_payload(h264_stream_t* h, bs_t* b, int payloadType, int payloadSize)
{
    ...
    for ( i = 0; i < payloadSize; i++ )
        s->payload[i] = h264_bs_read_u(b, 8);
...
void h264_write_sei_payload(h264_stream_t* h, bs_t* b, int payloadType, int payloadSize)
{
    ...
    for ( i = 0; i < s->payloadSize; i++ )
        h264_bs_write_u(b, 8, s->payload[i]);
}
